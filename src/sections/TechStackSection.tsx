...
export default function TechStackSection() {
  return (
    <section id="stack" aria-label="Technical Stack — Languages, Frameworks, Databases & Tools" className="relative py-16 sm:py-32 px-6 sm:px-8 md:px-16 lg:px-20 bg-black min-h-screen flex flex-col justify-center overflow-hidden">
      <FadingVideo
      ...
      <div className="relative z-10 container mx-auto">
        <span className="text-sm font-body text-white/40 mb-4 sm:mb-6 block tracking-widest uppercase">// Technical Arsenal</span>
        <h2 className="font-heading italic text-white text-4xl sm:text-5xl md:text-6xl tracking-[-2px] mb-8 sm:mb-16">The Stack</h2>
        ...
        <div className="flex flex-col gap-8 sm:gap-16">
          {[
            ...
            <div key={section.category}>
              <h3 className="text-lg sm:text-xl text-white/70 font-body mb-4 sm:mb-6 border-b border-white/10 pb-3 sm:pb-4">{section.category}</h3>
              <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-4">
                {section.skills.map((skill, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      className="group relative h-20 sm:h-32 liquid-glass rounded-xl sm:rounded-2xl p-3 sm:p-5 border border-white/5 overflow-hidden flex flex-col justify-end"
                    >
                      <div className="absolute inset-0 bg-white/0 group-hover:bg-white/[0.02] transition-colors duration-500" />

                      <div className="relative z-10 transition-transform duration-500 sm:group-hover:-translate-y-6 flex flex-col gap-1 sm:gap-2">
                        <Icon className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-white/30 group-hover:text-white/80 transition-colors duration-500" />
                        <h4 className="text-xs sm:text-xl font-heading italic text-white/90 leading-tight">{skill.name}</h4>
                      </div>

                      <div className="hidden sm:block absolute bottom-5 left-5 right-5 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
