...
export default function HackathonsSection() {
  ...
  return (
    <section id="hackathons" aria-label="Hackathon Experience & Competitive Achievements" className="relative py-16 sm:py-32 px-6 sm:px-8 md:px-16 lg:px-20 bg-black min-h-screen flex flex-col justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto">
        <div className="mb-8 sm:mb-16">
          <span className="text-sm font-body text-white/40 mb-4 sm:mb-6 block tracking-widest uppercase">// COMPETITIVE BUILD EXPERIENCE</span>
          <h2 className="font-heading italic text-white text-4xl sm:text-5xl md:text-6xl tracking-[-2px]">Hackathons &amp; Prototyping</h2>
          <p className="mt-3 sm:mt-4 text-xs sm:text-base text-white/50 max-w-2xl font-body font-light">Designing, engineering, and shipping fully functional systems under intense timeline constraints.</p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-12 items-stretch">
          <div className="lg:col-span-5 flex flex-col gap-3 sm:gap-4 lg:max-h-[640px]">
            <span className="text-xs text-white/30 uppercase tracking-widest font-mono hidden lg:block">Select Showcase</span>
            <div className="flex lg:flex-col gap-2 sm:gap-3 overflow-x-auto lg:overflow-y-auto lg:overflow-x-hidden pb-3 lg:pb-0 pr-0 lg:pr-1 snap-x lg:snap-y scrollbar-hide">
              {hackathons.map((h, index) => {
                const isActive = activeHackathon === index;
                return (
                  <button key={h.id} onClick={() => setActiveHackathon(index)} className="text-left flex-shrink-0 lg:flex-shrink snap-start w-56 sm:w-72 lg:w-full relative px-4 sm:px-6 py-3 sm:py-5 rounded-[1.25rem] border transition-all duration-300 focus:outline-none focus:ring-0 cursor-pointer text-white" style={{ borderColor: isActive ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.02)', background: isActive ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.01)' }}>
                    ...
                    <div className="flex justify-between items-start mb-1 sm:mb-2">
                      <span className="text-[9px] sm:text-[10px] tracking-widest font-mono text-white/40 uppercase">{h.id}</span>
                      <span className="text-[10px] sm:text-xs font-mono text-white/30 flex items-center gap-1"><Calendar className="w-3 h-3 opacity-60" /> {h.year}</span>
                    </div>
                    <h3 className="text-sm sm:text-lg font-heading italic text-white/95 line-clamp-1 mb-1 sm:mb-2">{h.title}</h3>
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      <div className="flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-white/5 border border-white/10"><Trophy className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white/60" /></div>
                      <span className="text-[10px] sm:text-xs text-white/60 font-body font-light truncate">{h.award}</span>
                    </div>
                  </button>
                );
              })}
          ...
          {/* Detail panel — compact on mobile */}
          <div className="lg:col-span-7 flex flex-col">
            <motion.div key={activeHackathon} initial={{ opacity: 0, y: 20, filter: 'blur(8px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} className="liquid-glass border border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-8 h-full min-h-0 sm:min-h-[540px] lg:min-h-[640px] flex flex-col justify-between relative overflow-hidden">
              <div>
                {/* Header */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-white/5 pb-4 sm:pb-6 mb-4 sm:mb-6">
                  <div>
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                      <span className="text-[9px] sm:text-[10px] font-mono tracking-widest bg-white/[0.04] border border-white/5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-white/50 uppercase">{hack.sub}</span>
                      <span className="text-[9px] sm:text-[10px] font-mono tracking-widest bg-white/10 text-white/90 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full uppercase flex items-center gap-1 border border-white/10"><Sparkles className="w-2.5 h-2.5" />{hack.award}</span>
                    </div>
                    <h4 className="font-heading italic text-xl sm:text-2xl md:text-3xl text-white tracking-tight leading-none">{hack.title}</h4>
                  </div>
                  <div className="flex-shrink-0 sm:text-right">
                    <span className="text-[10px] sm:text-xs text-white/30 font-mono">Project Built</span>
                    <span className="text-xs sm:text-sm font-heading italic text-white whitespace-nowrap block mt-0.5">{hack.project}</span>
                  </div>
                </div>

                {/* Role badge — compact */}
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 bg-white/[0.02] border border-white/5 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 w-fit">
                  <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-green-500 animate-pulse" />
                  <span className="text-[10px] sm:text-xs font-mono text-white/50 uppercase tracking-widest">Role:</span>
                  <span className="text-[10px] sm:text-xs font-body font-medium text-white">{hack.role}</span>
                </div>

                {/* Impact points — compact text on mobile */}
                <div className="mb-4 sm:mb-8">
                  <h5 className="text-[10px] sm:text-xs font-mono text-white/40 uppercase tracking-widest mb-2 sm:mb-4">Key Innovation &amp; Architecture Impact</h5>
                  <ul className="space-y-2 sm:space-y-4">
                    {hack.impact.map((point, index) => (
                      <li key={index} className="flex gap-2 sm:gap-3 items-start text-[11px] sm:text-sm text-white/70 font-body font-light leading-relaxed">
                        <span className="flex-shrink-0 w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-white/30 transform translate-y-1.5 sm:translate-y-2" />
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
              ...
              {/* Footer — tech + CTA */}
              <div className="border-t border-white/5 pt-4 sm:pt-6 mt-2 sm:mt-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    {hack.tech.slice(0, 6).map((tag) => (<span key={tag} className={`border text-[8px] sm:text-[10px] font-mono px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full transition-colors duration-200 ${getTagStyle(tag)}`}>{tag}</span>))}
                    {hack.tech.length > 6 && <span className="text-[8px] sm:text-[10px] font-mono text-white/30 px-1 self-center">+{hack.tech.length - 6}</span>}
                  </div>
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    {hack.liveUrl && (<a href={hack.liveUrl} target="_blank" rel="noopener noreferrer" className="liquid-glass-strong hover:bg-white/10 active:scale-95 transition-all text-[10px] sm:text-xs font-semibold py-2 sm:py-3 px-4 sm:px-5 rounded-full flex items-center justify-center gap-1.5 cursor-pointer text-white">View App <ArrowUpRight className="w-3 h-3" /></a>)}
                    {hack.github && (<a href={hack.github} target="_blank" rel="noopener noreferrer" className="liquid-glass-strong hover:bg-white/10 active:scale-95 transition-all text-[10px] sm:text-xs font-semibold py-2 sm:py-3 px-4 sm:px-5 rounded-full flex items-center justify-center gap-1.5 cursor-pointer text-white">GitHub <ArrowUpRight className="w-3 h-3" /></a>)}
                    {!hack.liveUrl && !hack.github && (<button onClick={() => document.getElementById('archives')?.scrollIntoView({ behavior: 'smooth' })} className="liquid-glass-strong hover:bg-white/10 active:scale-95 transition-all text-[10px] sm:text-xs font-semibold py-2 sm:py-3 px-4 sm:px-5 rounded-full flex items-center justify-center gap-1.5 cursor-pointer text-white">View App <ArrowUpRight className="w-3 h-3" /></button>)}
                  </div>
                </div>
              </div>
            </motion.div>
